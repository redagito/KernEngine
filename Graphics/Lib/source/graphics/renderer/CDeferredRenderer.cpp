...
#include "graphics/graphics/renderer/CRenderBuffer.h"
...
void CDeferredRenderer::draw(const IScene &scene, const ICamera &camera,
                                     ...
                                     ISceneQuery &query)
{
  ...
  CShaderProgram *geometryPassShader =
      manager.getShaderProgram(m_geometryPassShaderId);
  ...
  geometryPassShader->setUniform(viewMatrixUniformName,
                                 m_transformer.getViewMatrix());
  ...
  while (query.hasNextObject())
  {
    ...
    else
    {
      ...
      // Forward draw call
      draw(mesh, m_transformer.getTranslationMatrix(),
           m_transformer.getRotationMatrix(), m_transformer.getScaleMatrix(),
           material, manager, geometryPassShader);
    }
  }
...
void CDeferredRenderer::shadowMapPass(const IScene &scene,
                                      const ICamera &camera,
                                      const IWindow &window,
                                      const IGraphicsResourceManager &manager)
{
  CShaderProgram *shadowMapPassShader =
      manager.getShaderProgram(m_shadowMapPassShaderId);
  ...
  // Send view/projection to default shader
  shadowMapPassShader->setUniform(viewMatrixUniformName,
  ...
  while (query.hasNextObject())
  {
    ...
    else
    {
      ...
      // Forward draw call
      draw(mesh, transformer.getTranslationMatrix(),
           transformer.getRotationMatrix(), transformer.getScaleMatrix(),
           material, manager, shadowMapPassShader);
    }
  }
...
void CDeferredRenderer::shadowCubePass(const IScene &scene,
                                       const ICamera &camera,
                                       const IWindow &window,
                                       const IGraphicsResourceManager &manager)
{
  CShaderProgram *shadowCubePassShader =
      manager.getShaderProgram(m_shadowCubePassShaderId);
  ...
  shadowCubePassShader->setUniform(lightPositionUniformName,
                                   camera.getPosition());
  ...
  for (unsigned int i = 0; i < 6; ++i)
  {
    ...
    shadowCubePassShader->setUniform(projectionMatrixUniformName,
    ...
    while (query.hasNextObject())
    {
      ...
      else
      {
        ...
        draw(mesh, transformer.getTranslationMatrix(),
             transformer.getRotationMatrix(), transformer.getScaleMatrix(),
             material, manager, shadowCubePassShader);
      }
    }
  }
...
void CDeferredRenderer::lightPass(const IScene &scene, const ICamera &camera,
                                  const IWindow &window,
                                  const IGraphicsResourceManager &manager,
                                  ISceneQuery &query)
{
  ...
  glClearColor(ambientColor.x * ambientIntensity,
...
void CDeferredRenderer::pointLightPass(const IScene &scene,
                                       const ICamera &camera,
                                       const IWindow &window,
                                       const IGraphicsResourceManager &manager,
                                       ISceneQuery &query)
{
  // Point light pass
  CShaderProgram *pointLightPassShader =
      manager.getShaderProgram(m_pointLightPassShaderId);
  ...
  while (query.hasNextPointLight())
  {
    ...
    else
    {
      ...
      // 89.55 too big.
      glm::mat4 shadowProj =
          glm::perspective(89.54f, 1.0f, 0.01f, radius * 1.5f);
      ...
      pointLightPassShader->setUniform(depthTextureUniformName,
      ...
      // Set screen size
      pointLightPassShader->setUniform(screenWidthUniformName,
                                       (float)window.getWidth());
      pointLightPassShader->setUniform(screenHeightUniformName,
                                       (float)window.getHeight());

      // Camera position
      pointLightPassShader->setUniform(cameraPositionUniformName,
                                       camera.getPosition());

      // Inverse view-projection
      pointLightPassShader->setUniform(
          inverseViewProjectionMatrixUniformName,
...
void CDeferredRenderer::directionalLightPass(
    const IScene &scene, const ICamera &camera, const IWindow &window,
    const IGraphicsResourceManager &manager, ISceneQuery &query)
{
  ...
  {
    LOG_ERROR(
        "Shader program for directional light pass could not be retrieved.");
    return;
  }
  ...
  while (query.hasNextDirectionalLight())
  {
    ...
    if (!scene.getDirectionalLight(directionalLightId, direction, color,
                                   intensity, castsShadow))
    {
      LOG_ERROR(
          "Failed to retrieve directional light data from point light id %i.",
          directionalLightId);
    }
    else
    {
      ...
      glm::mat4 shadowView = glm::lookAt(
          glm::vec3(0), glm::normalize(direction), glm::vec3(0.0f, 1.0f, 0.0f));
      glm::mat4 shadowProj =
      ...
      // Set texture with world space normal and specular power
      m_normalSpecularTexture->setActive(lightPassNormalSpecularTextureUnit);
      directionalLightPassShader->setUniform(
          normalSpecularTextureUniformName, lightPassNormalSpecularTextureUnit);
      ...
      directionalLightPassShader->setUniform(
          shadowViewProjectionBiasMatrixUniformName, shadowViewProjBiasMatrix);

      // Set directional light parameters
      directionalLightPassShader->setUniform(lightDirectionUniformName,
                                             direction);
      directionalLightPassShader->setUniform(lightColorUniformName, color);
      directionalLightPassShader->setUniform(lightIntensityUniformName,
                                             intensity);
      ::draw(*quadMesh);
    }
  }
...
void CDeferredRenderer::illuminationPass(
    const IScene &scene, const ICamera &camera, const IWindow &window,
    const IGraphicsResourceManager &manager, ISceneQuery &query)
{
  // Reset viewport
  ...
  CShaderProgram *illuminationShader =
      manager.getShaderProgram(m_illuminationPassShaderId);
  ...
  illuminationShader->setUniform(lightTextureUniformName,
                                 illuminationPassLightTextureUnit);
  ...
  illuminationShader->setUniform(depthTextureUniformName,
                                 illuminationPassDepthTextureUnit);

  // Screen parameters
  illuminationShader->setUniform(screenWidthUniformName,
                                 (float)window.getWidth());
  illuminationShader->setUniform(screenHeightUniformName,
                                 (float)window.getHeight());
...
void CDeferredRenderer::postProcessPass(
    const ICamera &camera, const IWindow &window,
    const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &texture)
{
  ...
  if ((camera.getFeatureInfo().godRayActive ||
       camera.getFeatureInfo().renderMode == RenderMode::GodRay))
  {
    // God ray pass 1
    m_postProcessPassFrameBuffer1.setActive(GL_FRAMEBUFFER);
    ...
    // God ray pass 2
    m_postProcessPassFrameBuffer2.setActive(GL_FRAMEBUFFER);
    godRayPass2(window, manager, m_postProcessPassTexture0,
  ...
  if (camera.getFeatureInfo().lenseFlareActive)
  {
    ...
    m_postProcessPassFrameBuffer0.setActive(GL_FRAMEBUFFER);
    lensFlarePass3(window, manager, m_postProcessPassTexture0,
                   m_postProcessPassTexture1);
...
void CDeferredRenderer::fxaaPass(const IWindow &window,
                                 const IGraphicsResourceManager &manager,
                                 const std::shared_ptr<CTexture> &texture)
{
  // Get fxaa shader
...
void CDeferredRenderer::gaussBlurVerticalPass(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &texture)
{
  // Get gauss shader
  CShaderProgram *shader =
      manager.getShaderProgram(m_gaussBlurVerticalShaderId);
  if (shader == nullptr)
  {
    LOG_ERROR(
        "Shader program for vertical gauss blur pass could not be retrieved.");
    return;
  }
  ...
  {
    LOG_ERROR(
        "Mesh object for vertical gauss blur pass could not be retrieved.");
    return;
  }

  // Input texture
  texture->setActive(gaussBlurVerticalPassInputTextureUnit);
  shader->setUniform(sceneTextureUniformName,
                     gaussBlurVerticalPassInputTextureUnit);
...
void CDeferredRenderer::gaussBlurHorizontalPass(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &texture)
{
  // Get gauss shader
  CShaderProgram *shader =
      manager.getShaderProgram(m_gaussBlurHorizontalShaderId);
  if (shader == nullptr)
  {
    LOG_ERROR("Shader program for horizontal gauss blur pass could not be "
              "retrieved.");
    return;
  }
  ...
  {
    LOG_ERROR(
        "Mesh object for horizontal gauss blur pass could not be retrieved.");
    return;
  }
  ...
  shader->setUniform(sceneTextureUniformName,
                     gaussBlurHoriontalPassInputTextureUnit);
...
void CDeferredRenderer::depthOfFieldPass(
    const ICamera &camera, const IWindow &window,
    const IGraphicsResourceManager &manager,
    ...
    const std::shared_ptr<CTexture> &blurTexture)
{
  ...
  shader->setUniform(focusNearUniformName,
                     camera.getFeatureInfo().dofNearFocus);
...
void CDeferredRenderer::displayPass(const IWindow &window,
                                    const IGraphicsResourceManager &manager,
                                    const std::shared_ptr<CTexture> &texture)
{
  // Set main FBO active
...
void CDeferredRenderer::passthroughPass(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &texture)
{
  // Reset viewport
  ...
  // Get display shader
  CShaderProgram *displayShader =
      manager.getShaderProgram(m_displayPassShaderId);
  ...
  displayShader->setUniform(sceneTextureUniformName,
                            displayPassSceneTextureUnit);
...
void CDeferredRenderer::godRayPass1(const IWindow &window,
                                    const IGraphicsResourceManager &manager,
                                    const std::shared_ptr<CTexture> &texture)
{
  // Get shader
...
void CDeferredRenderer::godRayPass2(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &sceneTexture,
...
void CDeferredRenderer::visualizeDepthPass(
    const ICamera &camera, const IWindow &window,
    const IGraphicsResourceManager &manager)
{
  // Get shader
  CShaderProgram *shader =
      manager.getShaderProgram(m_visualizeDepthPassShaderId);
  if (shader == nullptr)
  {
    LOG_ERROR(
        "Shader program for depth visualization pass could not be retrieved.");
    return;
  }
  ...
  {
    LOG_ERROR(
        "Mesh object for depth visualization pass could not be retrieved.");
    return;
  }

  // Depth texture
  m_depthTexture->setActive(visualizeDepthPassDepthTextureUnit);
  shader->setUniform(depthTextureUniformName,
                     visualizeDepthPassDepthTextureUnit);
...
void CDeferredRenderer::vignetteBlurPass(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &texture)
{
...
void CDeferredRenderer::bloomPass1(const IWindow &window,
                                   const IGraphicsResourceManager &manager,
                                   const std::shared_ptr<CTexture> &texture)
{
...
void CDeferredRenderer::bloomPass2(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &sceneTexture,
...
void CDeferredRenderer::lensFlarePass(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &sceneTexture)
{
...
void CDeferredRenderer::lensFlarePass2(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &sceneTexture)
{
...
void CDeferredRenderer::lensFlarePass3(
    const IWindow &window, const IGraphicsResourceManager &manager,
    const std::shared_ptr<CTexture> &sceneTexture,
...
void CDeferredRenderer::celPass(const IWindow &window,
                                const IGraphicsResourceManager &manager,
                                const std::shared_ptr<CTexture> &sceneTexture)
{
...
void CDeferredRenderer::toneMapPass(const IWindow &window,
                                    const IGraphicsResourceManager &manager,
                                    const std::shared_ptr<CTexture> &texture)
{
  // Get tone map shader
...
void CDeferredRenderer::draw(CMesh *mesh, const glm::mat4 &translation,
...
bool CDeferredRenderer::initShadowCubePass(IResourceManager &manager)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_R32F, 1024, 1024, 0,
                 GL_RED, GL_FLOAT, NULL);
  }

  m_shadowCubeTexture = std::make_shared<CTexture>(
      textureId, false, 1024, 1024, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT);
  ...
  LOG_INFO("Shadow cube buffer state: %s.",
           m_shadowCubeBuffer.getState().c_str());
...
bool CDeferredRenderer::initShadowMapPass(IResourceManager &manager)
{
  ...
  LOG_INFO("Shadow map buffer state: %s.",
           m_shadowMapBuffer.getState().c_str());
...
{
  // Point light shader
  std::string pointLightPassShaderFile(
      "data/shader/deferred/point_light_pass.ini");
  ...
  std::shared_ptr<CRenderBuffer> depthBuffer =
      std::make_shared<CRenderBuffer>();
...
bool CDeferredRenderer::initDirectionalLightPass(IResourceManager &manager)
{
  ...
  std::string directionalLightPassShaderFile(
      "data/shader/deferred/directional_light_pass.ini");
  m_directionalLightPassShaderId =
      manager.loadShader(directionalLightPassShaderFile);
...
bool CDeferredRenderer::initIlluminationPass(IResourceManager &manager)
{
  ...
  std::string illuminationPassShaderFile =
      "data/shader/deferred/illumination_pass.ini";
  ...
  m_illumationPassFrameBuffer.attach(m_illuminationPassTexture,
...
bool CDeferredRenderer::initPostProcessPass(IResourceManager &manager)
{
  ...
  m_postProcessPassTexture0->setParameter(GL_TEXTURE_WRAP_R,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture0->setParameter(GL_TEXTURE_WRAP_S,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture0->setParameter(GL_TEXTURE_WRAP_T,
                                          GL_CLAMP_TO_BORDER);
  ...
  m_postProcessPassTexture1->setParameter(GL_TEXTURE_WRAP_R,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture1->setParameter(GL_TEXTURE_WRAP_S,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture1->setParameter(GL_TEXTURE_WRAP_T,
                                          GL_CLAMP_TO_BORDER);
  ...
  m_postProcessPassTexture2->setParameter(GL_TEXTURE_WRAP_R,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture2->setParameter(GL_TEXTURE_WRAP_S,
                                          GL_CLAMP_TO_BORDER);
  m_postProcessPassTexture2->setParameter(GL_TEXTURE_WRAP_T,
                                          GL_CLAMP_TO_BORDER);

  m_postProcessPassFrameBuffer0.attach(m_postProcessPassTexture0,
                                       GL_COLOR_ATTACHMENT0);
  m_postProcessPassFrameBuffer1.attach(m_postProcessPassTexture1,
                                       GL_COLOR_ATTACHMENT0);
  m_postProcessPassFrameBuffer2.attach(m_postProcessPassTexture2,
                                       GL_COLOR_ATTACHMENT0);
...
{
  // Depth of field shader
  std::string gaussBlurVerticalShaderFile =
      "data/shader/post/gauss_blur_vertical_pass.ini";
...
{
  // Depth of field shader
  std::string gaussBlurHorizontalShaderFile =
      "data/shader/post/gauss_blur_horizontal_pass.ini";
  m_gaussBlurHorizontalShaderId =
      manager.loadShader(gaussBlurHorizontalShaderFile);
  // Check if ok
