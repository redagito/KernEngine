...
CScreenSpacePass::CScreenSpacePass()
{
  ...
  m_quad.reset(
      new CMesh(vertices, indices, normals, uvs, EPrimitiveType::Point));
}

bool CScreenSpacePass::init(const std::string &shaderFile,
                            IResourceManager *manager)
{
  LOG_INFO("Initializing screen space pass with shader %s.",
           shaderFile.c_str());
  m_shaderId = manager->loadShader(shaderFile);
  if (m_shaderId == invalidResource)
  {
    LOG_ERROR("Failed to initialize the screen space pass from shader %s.",
              shaderFile.c_str());
    return false;
  }
  return true;
...
void CScreenSpacePass::draw(const IGraphicsResourceManager *manager,
                            CFrameBuffer *fbo, CTexture *texture0,
                            CTexture *texture1, CTexture *texture2,
                            CTexture *texture3)
{
  if (m_shaderId == invalidResource)
  {
