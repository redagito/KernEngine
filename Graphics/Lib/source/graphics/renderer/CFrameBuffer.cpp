...
void CFrameBuffer::attach(const std::shared_ptr<CTexture> &texture,
                          GLenum attachment, ETextureSemantic semantic)
{
  // Bind
  assert(m_valid);
  glBindFramebuffer(GL_FRAMEBUFFER, m_fboId);
...
void CFrameBuffer::setDefaultActive() { glBindFramebuffer(GL_FRAMEBUFFER, 0); }

void CFrameBuffer::attach(const std::shared_ptr<CRenderBuffer> &renderBuffer,
                          GLenum attachment)
{
  // Bind
  assert(m_valid);
