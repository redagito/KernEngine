...
void CGraphicsSystem::setActiveRenderer(const std::string &renderer)
{
  ...
  else
  {
    // Use default
    LOG_WARNING(
        "Invalid or unknown renderer type %s. Setting renderer to default.",
        renderer.c_str());
    m_activeRenderer = m_deferredRenderer.get();
  }
}
...
void CGraphicsSystem::draw(IWindow &window)
{
  ...
  {
    m_activeRenderer->draw(*m_activeScene, *m_activeCamera, window,
                           *m_resourceManager);
  }
  ...
  if (m_drawDebugOverlay)
  {
    ...
    m_debugInfo.setValue("Camera x",
                         std::to_string(m_activeCamera->getPosition().x));
    m_debugInfo.setValue("Camera y",
                         std::to_string(m_activeCamera->getPosition().y));
    m_debugInfo.setValue("Camera z",
                         std::to_string(m_activeCamera->getPosition().z));
    m_debugInfo.setValue("FPS", std::to_string(m_lastFrameCount));
    m_debugInfo.setValue(
        "Culled objects",
