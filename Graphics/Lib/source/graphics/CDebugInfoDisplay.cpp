                ...
                std::unique_ptr<CShaderProgram> &resultShader)
{
  ...
  {
    LOG_ERROR("Failed to load shader from %s, invalid id returned.",
              path.c_str());
  }
  ResourceId vertexShaderId, tessellationControlShaderId,
      tessellationEvaluationShaderId, geometryShaderId, fragmentShaderId;
  if (!resourceManager.getShader(
          shaderId, vertexShaderId, tessellationControlShaderId,
          tessellationEvaluationShaderId, geometryShaderId, fragmentShaderId))
  {
  ...
  resultShader.reset(new CShaderProgram(&vertexShader, nullptr, nullptr,
...
CDebugInfoDisplay::CDebugInfoDisplay(IResourceManager &resourceManager)
{
  ...
  loadShader("data/shader/shader_display_text.ini", resourceManager,
             m_textShader);
  loadShader("data/shader/shader_display_overlay.ini", resourceManager,
             m_overlayShader);
...
void textToBuffers(const std::string &text, unsigned int x, unsigned int y,
                   unsigned int size, std::vector<float> &vertices,
...
void CDebugInfoDisplay::draw(const CDebugInfo &info, bool transparent)
{
  ...
  overlayVertices.push_back((float)hOffsetKeys);
  overlayVertices.push_back(
      (float)(vOffsetKeyValues - m_fontSize * info.getValues().size()));
  overlayVertices.push_back(800.f);
  overlayVertices.push_back((float)vOffsetKeyValues);
  ...
  {
    textToBuffers(line, hOffsetLog, vOffsetLog - m_fontSize * ++lineCount,
  ...
  {
    ++valueCount;
    textToBuffers(keyValuePair.first, hOffsetKeys,
                  vOffsetKeyValues - m_fontSize * valueCount, m_fontSize,
                  vertices, UVs);
