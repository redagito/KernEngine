...
bool CSceneQuery::hasNextObject() const
{
  return m_nextObjectIndex < m_visibleObjects.size();
}
...
void CSceneQuery::addObject(SceneObjectId id)
{
  m_visibleObjects.push_back(id);
}

void CSceneQuery::addPointLight(SceneObjectId id)
{
  m_visiblePointLights.push_back(id);
}
