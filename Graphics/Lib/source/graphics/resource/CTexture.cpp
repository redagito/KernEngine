...
CTexture::CTexture(GLint id, bool hasMipmaps, unsigned int width,
                   unsigned int height, GLint format, GLenum externalFormat)
{
  m_valid = true;
...
void CTexture::resize(unsigned int width, unsigned int height)
{
  ...
  LOG_DEBUG("Texture resize from %u, %u to %u, %u.", m_width, m_height, width,
            height);
  glBindTexture(GL_TEXTURE_2D, m_textureId);
  glTexImage2D(GL_TEXTURE_2D, 0, m_format, width, height, 0, m_externalFormat,
                    ...
                    unsigned int height, GLint format, bool createMipmaps)
{
  ...
  if (image.empty())
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, m_externalFormat,
                 type, nullptr);
  }
  else
  {
