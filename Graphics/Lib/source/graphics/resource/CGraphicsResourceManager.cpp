...
void CGraphicsResourceManager::onAttach(IResourceManager *resourceManager)
{
  ...
  if (std::find(m_registeredManagers.begin(), m_registeredManagers.end(),
                resourceManager) == m_registeredManagers.end())
  {
...
void CGraphicsResourceManager::notify(EResourceType type, ResourceId id,
...
void CGraphicsResourceManager::initDefaultTextures()
{
  ...
  m_defaultDiffuseTexture.reset(
      new CTexture({238, 18, 137}, 1, 1, EColorFormat::RGB24, 0));
  ...
  m_defaultNormalTexture.reset(
      new CTexture({128, 128, 255}, 1, 1, EColorFormat::RGB24, 0));

  // Default specular texture is black (no specular highlights)
  m_defaultSpecularTexture.reset(
      new CTexture({0}, 1, 1, EColorFormat::GreyScale8));
  ...
  m_defaultAlphaTexture.reset(
      new CTexture({255}, 1, 1, EColorFormat::GreyScale8));
}
...
TShaderObject<GL_VERTEX_SHADER> *
CGraphicsResourceManager::getVertexShaderObject(ResourceId id) const
{
  // Invalid id
  if (id == invalidResource)
  {
...
TShaderObject<GL_TESS_CONTROL_SHADER> *
CGraphicsResourceManager::getTessControlShaderObject(ResourceId id) const
{
  // Invalid id
  if (id == invalidResource)
  {
...
TShaderObject<GL_TESS_EVALUATION_SHADER> *
CGraphicsResourceManager::getTessEvalShaderObject(ResourceId id) const
{
  // Invalid id
  if (id == invalidResource)
  {
...
TShaderObject<GL_GEOMETRY_SHADER> *
CGraphicsResourceManager::getGeometryShaderObject(ResourceId id) const
{
  // Invalid id
  if (id == invalidResource)
  {
...
TShaderObject<GL_FRAGMENT_SHADER> *
CGraphicsResourceManager::getFragmentShaderObject(ResourceId id) const
{
  // Invalid id
  if (id == invalidResource)
  {
...
bool CGraphicsResourceManager::loadVertexShader(
    ResourceId id, IResourceManager *resourceManager)
{
  // Unused id
  if (id == invalidResource)
  {
...
bool CGraphicsResourceManager::loadTessControlShader(
    ResourceId id, IResourceManager *resourceManager)
{
  // Unused id
  if (id == invalidResource)
...
bool CGraphicsResourceManager::loadTessEvalShader(
    ResourceId id, IResourceManager *resourceManager)
{
  // Unused id
  if (id == invalidResource)
...
bool CGraphicsResourceManager::loadGeometryShader(
    ResourceId id, IResourceManager *resourceManager)
{
  // Unused id
  if (id == invalidResource)
...
bool CGraphicsResourceManager::loadFragmentShader(
    ResourceId id, IResourceManager *resourceManager)
{
  // Unused id
  if (id == invalidResource)
...
void CGraphicsResourceManager::handleImageEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
  ...
  case EListenerEvent::Create:
    ...
    // Create new texture
    m_textures[id] = std::move(
        std::unique_ptr<CTexture>(new CTexture(data, width, height, format)));
    break;
...
void CGraphicsResourceManager::handleMeshEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
  ...
  case EListenerEvent::Create:
    ...
    // Create new texture
    m_meshes[id] = std::move(std::unique_ptr<CMesh>(
        new CMesh(vertices, indices, normals, uvs, type)));
    break;
...
void CGraphicsResourceManager::handleMaterialEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
  ResourceId diffuse;
  ResourceId normal;
  ...
  case EListenerEvent::Create:
    ...
    if (!resourceManager->getMaterial(id, diffuse, normal, specular, glow,
                                      alpha))
    {
    ...
    // Create new material
    m_materials[id] = std::move(std::unique_ptr<CMaterial>(new CMaterial(
        getTexture(diffuse), getTexture(normal), getTexture(specular),
  ...
  case EListenerEvent::Change:
    ...
    m_materials.at(id)->init(getTexture(diffuse), getTexture(normal),
                             getTexture(specular), getTexture(glow),
                             getTexture(alpha));
    break;
...
void CGraphicsResourceManager::handleModelEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
  ResourceId mesh;
  ResourceId material;
...
void CGraphicsResourceManager::handleShaderEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
  ResourceId vertex;
  ...
  case EListenerEvent::Create:
    ...
    if (!resourceManager->getShader(id, vertex, tessControl, tessEval, geometry,
                                    fragment))
    {
      // TODO Replace assert with log statement and global error handler
    ...
    m_shaderPrograms[id] = std::move(std::unique_ptr<CShaderProgram>(
        new CShaderProgram(getVertexShaderObject(vertex),
                           getTessControlShaderObject(tessControl),
                           getTessEvalShaderObject(tessEval),
                           getGeometryShaderObject(geometry),
...
void CGraphicsResourceManager::handleStringEvent(
    ResourceId id, EListenerEvent event, IResourceManager *resourceManager)
{
