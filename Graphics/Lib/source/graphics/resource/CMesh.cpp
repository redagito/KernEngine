...
CMesh::CMesh(const std::vector<float> &vertices,
             ...
             EPrimitiveType type)
    : m_vertices(nullptr), m_indices(nullptr), m_normals(nullptr),
      m_uvs(nullptr), m_vao(nullptr), m_type(EPrimitiveType::Invalid)
{
...
bool CMesh::init(const std::vector<float> &vertices,
                 const std::vector<unsigned int> &indices,
                 ...
                 const std::vector<float> &uvs, EPrimitiveType type)
{
  ...
  {
    m_normals->setActive();
    glVertexAttribPointer(normalDataShaderLocation, 3, GL_FLOAT, GL_FALSE, 0,
                          0);
...
const std::unique_ptr<CVertexBuffer> &CMesh::getVertexBuffer() const
{
  return m_vertices;
}

const std::unique_ptr<CIndexBuffer> &CMesh::getIndexBuffer() const
{
  return m_indices;
}

const std::unique_ptr<CVertexBuffer> &CMesh::getNormalBuffer() const
{
  return m_normals;
}

const std::unique_ptr<CVertexBuffer> &CMesh::getUVBuffer() const
{
  return m_uvs;
}
...
const CBoundingSphere &CMesh::getBoundingSphere() const
{
