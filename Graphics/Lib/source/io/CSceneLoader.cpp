                        ...
                        CAnimationWorld &animationWorld)
{
  ...
  {
    LOG_ERROR("Error while loading scene objects from scene file %s.",
              file.c_str());
    return false;
  }
  ...
  {
    LOG_ERROR("Error while loading point lights from scene file %s.",
              file.c_str());
    return false;
  }
  ...
  {
    LOG_ERROR("Error while loading ambient light from scene file &s.",
              file.c_str());
    return false;
  }
                                    ...
                                    CAnimationWorld &animationWorld)
{
  ...
  {
    LOG_INFO(
        "Missing or empty 'scene_objects' node. No scene objects are loaded.");
    return true;
  }
                                   ...
                                   CAnimationWorld &animationWorld)
{
  ...
  SceneObjectId objectId = scene.createObject(meshId, materialId, position,
                                              glm::quat(rotation), scale);

  // Load optional animation controllers
  if (!loadAnimationControllers(node["animations"], scene, animationWorld,
                                objectId, AnimationObjectType::Model))
  {
                                   ...
                                   CAnimationWorld &animationWorld)
{
  ...
  {
    LOG_INFO(
        "Missing or empty 'point_lights' node. No point lights are loaded.");
    return true;
  }
                                  ...
                                  CAnimationWorld &animationWorld)
{
  ...
  // Load optional animation controllers
  if (!loadAnimationControllers(node["animations"], scene, animationWorld,
                                objectId, AnimationObjectType::PointLight))
  {
    LOG_ERROR("Failed to load animation controller.");
    return false;
  }
                                         ...
                                         CAnimationWorld &animationWorld)
{
  ...
  if (node.empty())
  {
    LOG_INFO("Missing or empty 'directional_lights' node. No directional "
             "lights are loaded.");
    return true;
  }
...
bool CSceneLoader::loadAmbientLight(const Json::Value &node, IScene &scene)
{
  ...
  {
    LOG_INFO(
        "Missing or empty 'ambient_light' node. No ambient light is loaded.");
    return true;
  }
...
bool CSceneLoader::loadAnimationControllers(const Json::Value &node,
                                            IScene &scene,
                                            CAnimationWorld &animationWorld,
                                            SceneObjectId id,
...
bool CSceneLoader::loadAnimationController(const Json::Value &node,
                                           IScene &scene,
                                           CAnimationWorld &animationWorld,
                                           SceneObjectId id,
                                           AnimationObjectType type)
{
  ...
  else if (controllerType == "sine_translation")
  {
    ...
    animationWorld.addAnimationController(
        std::make_shared<CSineTranslationController>(id, type, scene,
...
bool CSceneLoader::load(const Json::Value &node, const std::string &name,
                        float &f)
{
