...
#include "graphics/game/state/CLoadState.h"
...
#include "graphics/game/state/CLoseState.h"
...
bool CEngine::init(const char *configFile)
{
  ...
  {
    LOG_WARNING(
        "Failed to load config file %s. Starting with default settings.",
        configFile);
  ...
  // directly
  m_inputProvider =
      std::make_shared<CGlfwInputProvider>(m_window->getGlfwHandle());
...
bool CEngine::initWindow(unsigned int width, unsigned int height,
...
bool CEngine::initGameSystem(const std::string &gameFile)
{
  ...
  m_gameSystem->addState("load",
                         new CLoadState("data/world/load_1.json", 10.f));
