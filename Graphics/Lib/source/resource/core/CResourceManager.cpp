...
ResourceId
CResourceManager::createMesh(const std::vector<float> &vertices,
...
ResourceId CResourceManager::loadMesh(const std::string &file)
{
  ...
  ResourceId meshId = createMesh(mesh.m_vertices, mesh.m_indices,
                                 mesh.m_normals, mesh.m_uvs, mesh.m_type);
...
ResourceId
CResourceManager::createImage(const std::vector<unsigned char> &imageData,
...
ResourceId CResourceManager::loadImage(const std::string &file,
                                       EColorFormat format)
{
  ...
  ResourceId imageId =
      createImage(image.m_data, image.m_width, image.m_height, image.m_format);
...
ResourceId CResourceManager::loadMaterial(const std::string &file)
{
  ...
  ResourceId materialId =
      createMaterial(material.m_base, material.m_normal, material.m_specular,
                     material.m_glow, material.m_alpha);
  if (materialId == invalidResource)
  {
    LOG_ERROR("Failed to create material resource id for material file %s.",
              file.c_str());
    return invalidResource;
  }
...
bool CResourceManager::getMaterial(ResourceId id, ResourceId &base,
                                   ResourceId &normal, ResourceId &specular,
...
ResourceId CResourceManager::loadModel(const std::string &file)
{
  ...
  {
    LOG_ERROR("Failed to create model resource id for model file %s.",
              file.c_str());
    return invalidResource;
  }
...
bool CResourceManager::getModel(ResourceId id, ResourceId &mesh,
                                ResourceId &material)
{
  // Retrieve from map
...
ResourceId CResourceManager::createShader(ResourceId vertex,
                                          ResourceId tessCtrl,
                                          ResourceId tessEval,
                                          ResourceId geometry,
                                          ResourceId fragment)
...
bool CResourceManager::getShader(ResourceId id, ResourceId &vertex,
                                 ResourceId &tessCtrl, ResourceId &tessEval,
                                 ResourceId &geometry,
...
ResourceId CResourceManager::loadShader(const std::string &file)
{
  ...
  {
    LOG_ERROR("Failed to create reasource id for shader file %s.",
              file.c_str());
    return invalidResource;
  }
...
void CResourceManager::notifyResourceListeners(EResourceType type,
                                               ResourceId id,
