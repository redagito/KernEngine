...
class CGraphicsResourceManager : public IGraphicsResourceManager,
...
private:
  ...
  */
  void handleImageEvent(ResourceId, EListenerEvent event,
                        IResourceManager *resourceManager);

  /**
  * \brief Handles resource events for mesh resources.
  */
  void handleMeshEvent(ResourceId, EListenerEvent event,
                       IResourceManager *resourceManager);

  /**
  * \brief Handles resource events for material resources.
  */
  void handleMaterialEvent(ResourceId, EListenerEvent event,
                           IResourceManager *resourceManager);

  /**
  * \brief Handles resource events for model resource.
  */
  void handleModelEvent(ResourceId, EListenerEvent event,
                        IResourceManager *resourceManager);

  /**
  * \brief Handles resource events for shader resources.
  */
  void handleShaderEvent(ResourceId, EListenerEvent event,
                         IResourceManager *resourceManager);
  ...
  */
  void handleStringEvent(ResourceId, EListenerEvent event,
                         IResourceManager *resourceManager);
  ...
  std::unordered_map<ResourceId,
                     std::unique_ptr<CTexture>> m_textures; /**< Maps image id
                                                               from resource
                                                               manager to GPU
                                                               side texture. */
  ...
  std::unique_ptr<CTexture> m_defaultDiffuseTexture =
      nullptr; /**< Default diffuse texture. */
  std::unique_ptr<CTexture> m_defaultNormalTexture =
      nullptr; /**< Default normal texture. */
  std::unique_ptr<CTexture> m_defaultSpecularTexture =
      nullptr; /**< Default specular texture. */
  std::unique_ptr<CTexture> m_defaultGlowTexture =
      nullptr; /**< Default glow texture. */
  std::unique_ptr<CTexture> m_defaultAlphaTexture =
      nullptr; /**< Default alpha texture. */
