...
public:
  ...
  */
   ...
   */
  virtual ResourceId createImage(const std::vector<unsigned char> &imageData,
                                 unsigned int width, unsigned int height,
                                 EColorFormat format) = 0;

  /**
  * \brief Loads image from file.
  */
  virtual ResourceId loadImage(const std::string &file,
                               EColorFormat format) = 0;

  /**
  * \brief Retrieves image data.
  */
  virtual bool getImage(ResourceId id, std::vector<unsigned char> &data,
                        unsigned int &width, unsigned int &height,
                        EColorFormat &format) const = 0;

  /**
  * \brief Creates material.
  */
  virtual ResourceId createMaterial(ResourceId baseImage,
                                    ResourceId normalImage,
                                    ResourceId specularImage,
                                    ResourceId glowImage,
  ...
  */
  virtual bool getMaterial(ResourceId id, ResourceId &baseImage,
                           ResourceId &alphaImage, ResourceId &normalImage,
                           ResourceId &specularImage,
  ...
  *
  * The file should usually contain refernces to the shader sources used for the
  * shading stages.
