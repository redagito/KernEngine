 ...
 */
template <typename SampleType, typename KeyType> class TSampler
{
public:
  typedef std::function<SampleType(
      KeyType key0, SampleType value0, KeyType key1, SampleType value1,
      KeyType key2, SampleType value2, KeyType key)> InterpolationFunction;

  /**
   * \brief Allocates initial sample vector storage.
   * Set the size according to expected number of samples.
   */
  TSampler(InterpolationFunction interpolFunc,
           unsigned int initSizeAlloc = 1024);
    ...
    if (m_samples.at(m_currIndex).key > key)
    {
      ...
      if (m_samples.size() > m_currIndex + 1)
      {
        ...
        return m_interpolationFunction(
            m_samples.at(m_currIndex - 1).key,
            m_samples.at(m_currIndex - 1).value, m_samples.at(m_currIndex).key,
            m_samples.at(m_currIndex).value, m_samples.at(m_currIndex + 1).key,
            m_samples.at(m_currIndex + 1).value, key);
      }
      else
      {
        return m_interpolationFunction(m_samples.at(m_currIndex - 2).key,
                                       m_samples.at(m_currIndex - 2).value,
                                       m_samples.at(m_currIndex - 1).key,
                                       m_samples.at(m_currIndex - 1).value,
                                       m_samples.at(m_currIndex).key,
                                       m_samples.at(m_currIndex).value, key);
      }
    }
    // Step next
    ++m_currIndex;
