...
bool bindValue(sqlite3_stmt *stmt, int id, const std::string &str)
{
    ...
    return sqlite3_bind_text(stmt, id, str.data(), (int)str.size(), SQLITE_STATIC) == SQLITE_OK;
}
...
bool prepareStatement(sqlite3 *db, const std::string &sql, sqlite3_stmt **stmt)
{
    ...
    return sqlite3_prepare_v2(db, sql.data(), (int)sql.size(), stmt, nullptr) == SQLITE_OK;
}