...
static bool resolveAlias(const std::string &alias, const std::string &path, std::string &str)
{
...
void CPathAliasRegistry::setAlias(const std::string &alias, const std::string &path)
{
    std::lock_guard<std::recursive_mutex> lg(m_pathAliasesMutex);
...
CUniformResourceIdentifier CPathAliasRegistry::resolveAliases(
    const CUniformResourceIdentifier &uri) const
{
    return uri;
}
