...
bool CObjModelLoader::load(const std::string &file)
{
  ...
  LOG_INFO("Loaded obj model %s with %u triangles.", file.c_str(),
           d_vertices.size() / 3);
  return true;
}
...
// Parsing logic
void CObjModelLoader::parseLine(const std::string &line,
                                std::vector<float> &tempVert,
                                std::vector<float> &tempNorm,
                                std::vector<float> &tempUV)
{
...
void CObjModelLoader::parseVertex(const std::string &line,
...
void CObjModelLoader::parseNormal(const std::string &line,
...
void CObjModelLoader::parseIndex(const std::string &line,
