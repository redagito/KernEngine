...
bool CProfiler::write(std::ostream &stream)
{
    ...
    for (const auto &data : s_profileData)
    {
        ...
        stream << "Total call time: " << data.second.totalCallTime << " [ms]" << std::endl;
        stream << "Min call time: " << data.second.minCallTime << " [ms]" << std::endl;
        stream << "Max call time: " << data.second.maxCallTime << " [ms]" << std::endl;
        stream << "Average call time: " << data.second.totalCallTime / data.second.callCount
               << " [ms]" << std::endl;
        stream << "---------------------------------" << std::endl;
    }
    return stream.good();
