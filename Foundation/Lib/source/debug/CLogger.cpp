...
void CLogger::log(ELogLevel level, const char *file, int line,
                  const char *function, const std::string &text)
{
  ...
  if (s_stream.is_open())
  {
    ...
    s_stream << toString(level) << " [" << file << ":" << line << ", "
             << function << "]: " << text << std::endl
             << std::flush;
  }
...
bool CLogger::initLogFile(const std::string &logFile)
{
  ...
  LOG_WARNING(
      "Multiple logger initialization calls are invalid and will be ignored.");
  return false;
}
