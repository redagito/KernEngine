...
void AMessageHandlerThread::attachHandler(const std::shared_ptr<IMessageHandler> &handler)
{
    ...
    auto iter = std::find(m_messageHandlers.begin(), m_messageHandlers.end(), handler);
    // Fail on debug
...
void AMessageHandlerThread::detachHandler(const std::shared_ptr<IMessageHandler> &handler)
{
    assert(handler != nullptr);
    if (handler == nullptr)
    {
...
void AMessageHandlerThread::waitForMessage(const std::shared_ptr<AMessage> &message)
{
