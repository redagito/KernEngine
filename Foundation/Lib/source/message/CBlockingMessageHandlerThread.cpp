...
void CBlockingMessageHandlerThread::addMessage(const std::shared_ptr<AMessage> &message)
{
    {
...
void CBlockingMessageHandlerThread::cancelMessage(const std::shared_ptr<AMessage> &message)
{
    std::lock_guard<std::mutex> lg(m_messagesMutex);
    // Remove-erase idiom
...
void CBlockingMessageHandlerThread::doWork()
{
    ...
    if (timeout > std::chrono::milliseconds{0})
    {
        ...
        m_messageAdded.wait_for(ul, timeout, [this] { return !m_messages.empty(); });
    }
    else
    {
        // Wait for message
        m_messageAdded.wait(ul, [this] { return !m_messages.empty(); });
    }
