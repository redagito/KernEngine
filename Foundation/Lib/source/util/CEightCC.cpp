...
std::string CEightCC::toString() const { return std::string{(const char *)&m_code, 8}; }

bool CEightCC::operator==(const CEightCC &rhs) const { return get() == rhs.get(); }

bool CEightCC::operator<(const CEightCC &rhs) const { return get() < rhs.get(); }

bool serialize(const CEightCC &code, std::ostream &os) { return serialize(code.get(), os); }
