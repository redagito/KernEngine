...
class IArchive
{
   public:
     ...
     */
    virtual const CUniformResourceIdentifier &getUniformResourceIdentifier() const = 0;

    /**
     * \return Array of file names in a directory inside the archive.
     */
    virtual std::vector<std::string> getFileNames(const std::string &pathInArchive) const = 0;

    /**
     * \return Array of directory names in a directory inside the archive.
     */
    virtual std::vector<std::string> getDirectoryNames(const std::string &pathInArchive) const = 0;
};
