...
* Obj files must be fully triangulated, hold only one model and contain normals
*and UVs.
*/
...
private:
  // Parse single line
  void parseLine(const std::string &, std::vector<float> &vert,
                 std::vector<float> &norm, std::vector<float> &uv);
  // Parse data
  ...
  void parseIndex(const std::string &, const std::vector<float> &,
