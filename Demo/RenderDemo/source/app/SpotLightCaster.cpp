...
void main() 
{
    ...
    float theta = dot(lightDir, normalize(-light.direction)); // Angle between fragment to light and light direction
    if (theta > light.outerCutOff)
    {
        ...
        vec3 diffuse = diff * diffuseTex * light.diffuse * attenuation * intensity;

        // specular
        vec3 viewDir = normalize(viewPos - fragPos);
        vec3 reflectDir = reflect(-lightDir, norm);
        float spec = pow(max(dot(viewDir, reflectDir), 0.f), material.shininess);
        vec3 specular = spec * specularTex * light.specular * attenuation * intensity;
    ...
    {
        // Outside light cone, only affected by ambient light
...
void main(){
    ...
    // Light
    m_spotLight.position = glm::vec3(0.f, 0.f, 8.f);
    m_spotLight.direction = glm::vec3(0.f, 0.f, -1.f);
    m_spotLight.innerCutOff = 12.5f;
    m_spotLight.outerCutOff= 17.f;
    m_spotLight.ambient = m_lightColor * 0.1f;
    m_spotLight.diffuse = m_lightColor * 0.8f;
    m_spotLight.specular = m_lightColor;
...
void SpotLightCaster::render()
{
    ...
    m_lightShader->set("model", glm::scale(glm::translate(m_spotLight.position), glm::vec3(0.3f)));
