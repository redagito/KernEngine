...
void CFreeFlightCameraController::setCamera(
    std::shared_ptr<IControllableCamera> camera)
{
  m_camera = camera;
}
...
void CFreeFlightCameraController::animateSequence(float dt)
{
  ...
  float ix = (m_sequenceTime - before.timestamp) /
             (after.timestamp - before.timestamp);
  glm::vec3 cameraPosition =
      before.position * (1.0f - ix) + after.position * ix;
  glm::vec3 cameraOrientation =
      before.orientation * (1.0f - ix) + after.orientation * ix;

  m_camera->lookAt(cameraPosition, cameraPosition + cameraOrientation,
                   glm::vec3(0, 1, 0));
  m_camera->getFeatureInfoForWrite().fogType =
      (before.fogActive ? FogType::Exp2 : FogType::None);
...
void CFreeFlightCameraController::handleKeyEvent(EKeyEventType type,
                                                 int keyCode)
{
...
void CFreeFlightCameraController::handleMouseButtonEvent(
...
{
  m_camera->setProjection(m_camera->getFieldOfView(), (float)width / height,
