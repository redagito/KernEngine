...
WinState::WinState(const std::string &sceneFile) : m_sceneFile(sceneFile) {}
...
bool WinState::init(IGraphicsSystem *graphicsSystem, IInputProvider *inputProvider, IResourceManager *resourceManager)
{
    ...
    m_camera = std::make_shared<FirstPersonCamera>(glm::vec3(-4.4f, 5.f, 14.5), glm::vec3(0.f, 0.f, 0.f),
...
bool WinState::update(float dtime)
{
...
const std::string &WinState::getNextState() const { return gameStr; }