...
Engine::~Engine() { m_gameSystem = nullptr; }
...
bool Engine::initWindow(unsigned int width, unsigned int height, const std::string &title)
{
...
bool Engine::initGameSystem(const std::string &gameFile)
{
    // Create and set game system
    m_gameSystem = std::make_shared<GameSystem>();

    // TODO Load from game file
    m_gameSystem->addState("load", new LoadState("data/world/load_1.json", 10.f));
    m_gameSystem->addState("title", new TitleState("data/world/intro_1.json"));
...
bool Engine::initDemo(const std::string &sceneFile)
{
    // Create and set game system
    m_gameSystem = std::make_shared<GameSystem>();
