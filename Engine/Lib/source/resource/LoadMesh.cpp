...
bool loadMeshFromObj(const std::string &file, SMesh &mesh)
{
    ...
    {
        loge("Failed to load mesh from file {}: {}", file, err);
        return false;
    }
    if (!err.empty())
    {
        loge("Error while loading mesh from file {}: {}", file, err);
        return false;
    }
    if (!warn.empty())
    {
        logw("Warning while loading mesh from file {}: {}", file, warn);
    }

    if (shapes.size() != 1)
    {
        loge("Loading of multiple meshes not supported: {}", file);
        return false;
    }

    auto shape = shapes.at(0);

    mesh.m_type = PrimitiveType::Triangle;
    /*
