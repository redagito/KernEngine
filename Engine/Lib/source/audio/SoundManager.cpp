...
std::shared_ptr<Sound> SoundManager::loadFromFile(const std::string& fileName)
{
    ...
    logi("Loading sound file {} from path {}", fileName, directory);
    if (extension == ".wav")
    {
...
std::shared_ptr<Sound> SoundManager::loadFromMp3(const std::string& fileName)
{
    mp3dec_t mp3;
    mp3dec_file_info_t info;
    if (mp3dec_load(&mp3, fileName.c_str(), &info, nullptr, nullptr) != 0)
    {
        throw std::runtime_error("Failed to load mp3");
    }

    auto sound = std::make_shared<Sound>(info.buffer, info.samples * sizeof(mp3d_sample_t), info.channels,
                                         static_cast<unsigned int>(sizeof(mp3d_sample_t) * 8), info.hz);
    free(info.buffer);
    return sound;
}